...
const DayTasks: FC<DayTasksProps> = (props: DayTasksProps) => {
  ...
  useEffect(() => {
    const calendarHeader = document.querySelector(
      ".rbc-time-header-content .rbc-allday-cell .rbc-row-bg > div"
    ) as HTMLElement;
    if (calendarHeader) {
      const dayString = props.day.toLocaleDateString(userLocale, {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
  ...
  const setTriggerButtonPosition = (event: React.MouseEvent<HTMLElement>) => {
    const triggerButton = document.getElementsByClassName(
      "trigger-button"
    )[0] as HTMLElement;
    ...
    <Box flex={1} onClick={setTriggerButtonPosition}>
        ...
        <PopoverContent>
          ...
          <PopoverBody>
            ...
            <Box display="flex" gap="12px">
              <Box
                bgColor={eventSelected?.event.category.color}
                fontSize={12}
                borderRadius="100%"
                px="6px"
                py="2px"
              ...
              <Box
                bgColor={getPriorityColor(eventSelected?.event.priority)}
                fontSize={12}
                borderRadius="100%"
                px="8px"
              >
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered autoFocus={false}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>Êtes-vous sûr de supprimer cet évènement ?</ModalBody>
          <ModalFooter>
